...
const useStyles = makeStyles((theme) => ({
  ...
  {/*TRX SISTEMAS INSERIDO O dentro do const handleChangeTab*/}
  ...
  const handleReopenTicket = async (id) => {
    ...
    const handleAcepptTicket = async (id) => {
        ...
        const msg = `{{ms}} *{{name}}*, mi nombre es *${user?.name}* y ahora procederé con tu atención.`;
        const message = {
            read: 1,
            fromMe: true,
            mediaUrl: "",
        ...
        <span className={classes.secondaryContentSecond} >
          {ticket.status === "pending" && (
            <ButtonWithSpinner
              ...
              size="small"
              loading={loading}
			  //TRX SISTEMAS INSERIDO O handleChangeTab
